$(document).ready(function () {
   ...
   if ((y == year && m == month) && !$('#EOMAppState').data('state') >= 1) {
      $('#syukkin').prop('disabled', false);
      $('#taikin').prop('disabled', false);
   } else {
      $('#syukkin').prop('disabled', true);
      $('#taikin').prop('disabled', true);
   }
...
function setWorkTime(day) {
   ...
   if ($.isNumeric(hh) && $.isNumeric(mm) && diff > 0) {
      ...
      $('#ip_worktime_' + day).attr('value', ('0' + hh).slice(-2) + ' : ' + ('0' + mm).slice(-2));
   } else {
      $('#worktime_' + day).html('00 : 00');
      $('ip_#worktime_' + day).attr('value', '00 : 00');
   }
}
